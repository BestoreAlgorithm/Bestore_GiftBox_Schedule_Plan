...
def lock_data_parse(df_last_produce, now_time):
    if df_last_produce.shape[0] == 0:  # TODO(新增标记)
    ...
    for i in range(df_last_produce.shape[0]):
        if (df_last_produce.loc[i, 'isLock']):
            ...
            day_dis_report = (report_date - the_first_date).days  # s_t 提报间隔时间 # TODO(改动标记)
            day_dis_produce = (produce_date - the_first_date).days  # o_t  排产间隔时间 # TODO（改动标记）
            print('提报间隔时间：\n {0}--排产间隔时间：\n {1}'.format(day_dis_report, day_dis_produce))
            if (day_dis_report == 0):
                day_dis_report = day_dis_report + 1
            # TODO(新增标记) :防止提报时间为0时，影响了优先级计算
...
def orders_f_data_parse(df_orders, lock, now_time):
    ...
    df_orders["packingPlanId"] = df_orders["packingPlanId"].astype(int)  # 将packingPlanId列转化为int # TODO (疑问标记?)
    df_orders["packingPlanSerialNum"] = df_orders["packingPlanSerialNum"].astype(int)  # TODO(疑问标记？)
    ...
    for i in range(df_orders.shape[0]):
        ...
        dis_day = (date_demand - the_first_date).days  # 分装需求时间间隔（天） o_t # TODO(改动标记)
        tos_day = (date_report - the_first_date).days  # 提报日期时间间隔（天）s_t  # TODO(改动标记)
        if dis_day >= 0:
            dis_day = dis_day + 1
            if dis_day >= 7:
                dis_day = 7
        else:
            print("ValueError：packingPlanWeekNum参数输入错误，需求预分装周次或要求出库周时间已过！")
        if tos_day == 0:
            tos_day = tos_day + 1
...
def I_0_data_inventory_parse(df_inventory):
    ...
    I_0["sample"] = I_0["sample"].astype(str)  # TODO(新增标记)
    I_0["warehouse"] = I_0["warehouse"].astype(str)  # TODO(新增标记)
    I_0["num"] = I_0["num"].astype(float)  # TODO(新增标记)
    print('[3]: (I_0)在库库存数据：\n {0}'.format(I_0.head()))
    return I_0
...
def arr_data_parse(df_arrival, now_time, arrive_interval_days):
    ...
    '''
    arrive_interval_days = arrive_interval_days + 1  # TODO(新增标记)：子件到货日期+1，用于转化为相对日期
    ...
    arr = pd.DataFrame(columns=['warehouse', 'sample', 'num', 't'])  # 创建一个arr数据帧
