...
{
	//存储json文件名的数组
	$file_name=array('priority','capacity','boms','calendar');
...
//json格式的字符串json化
function str_json($str_var){
	
	$str_var=json_decode($str_var);
	$js_var=json_encode($str_var);
...
if($content==''){
	...
	if (!json_empty()){
		...
		file_put_contents('jsons/ProducePlan.json', $content);
		//执行py文件并获得返回信息
		exec("/var/www/anaconda3/bin/python3 /var/www/html/7days.py",$arry,$info);
		//读取产生的结果文件
		$js_result=str_json(file_get_contents('7days_result.json'));
	}
